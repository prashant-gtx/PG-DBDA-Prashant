...
-- 7
select first_name, substring(first_name,-4) as last_four_char from employees;

-- 8
select last_name, instr(last_name, 'a') as position_of_a from employees;

-- 9
select job_id, instr(job_id, 'IT') as position_of_IT from employees;
select job_id, locate('IT',job_id) as position_of_IT from employees;
...
-- 14
select email, left(email,3) from employees;
...
select email, substring(email, locate('@', email) + 1, length(email)) as domain
from employees; -- error

-- 17
select phone_number, substring(phone_number,1,3) as country_code from employees;

-- 18
select first_name,last_name, first_name<>last_name as name_comparison from employees;

-- 19
select region_name, region_name<>'Asia' as compare from regions; 

-- 20
select concat(first_name,'-',last_name,'-',job_Id) from employees;

-- 21
select substring(email,1,instr(email, '@')) from employees;
...
-- 24
select trim(city),substring(city,1,3) from locations;
...
-- 29
select department_name from departments where department_name in('IT','Human Resources');
...
-- 32
select first_name,last_name from employees where department_id in(50, 60, 70);

-- 33
select substring(country_name,1,2), country_name, substring(country_name,-2) from countries;

-- 34
select * from Employees where instr(last_name, 'o') > length(last_name) / 2;

-- 35
select first_name, instr(first_name, 'a') as Position_of_a, instr(first_name, 'e') as Position_of_e
from Employees where first_name like '%a%' and first_name like '%e%';

-- 36
select * from employees where substring(email,locate('@',email));
...
-- 41
-- Write a query to display FIRST_NAME, LAST_NAME, and COUNTRY_NAME concatenated 
-- as a single string with a comma separating each.

select concat(employees.first_name,',', employees.last_name,',', countries.country_name) as full_name_country
from employees inner join countries ;

-- Write a query to display the FIRST_NAME in uppercase for employees whose 
-- SALARY is greater than 8000 and the LAST_NAME in lowercase for employees whose SALARY is less than 8000.
-- select upper(first_name),lower(last_name) from employees where salary > 8000 and salary <8000;
select first_name,last_name,salary,
if(salary > 8000,upper(first_name),lower(last_name)) as emp_comprision
from employees;
