...
-- --------------------------------------------------------------------------------------------------------------------------
select * from (select employee_id, first_name,department_id,salary, rank() over(partition by department_id order by salary desc) as salaryrank  from employees)ranked order by salaryrank asc;

use hr;

-- query to display top 3 highest payed employees by department
SELECT *
FROM (
    SELECT employee_id,concat(first_name,' ',last_name) full_name,job_id,salary,department_id,
...
select * from (select employee_id, concat(first_name,' ',last_name) full_name,
job_id,salary,department_id,row_number() over(partition by department_id order by salary desc) as sal_rank from employees)ranked;

-- For each employee, show their salary and the average salary of their department using AVG() as a window function.
select employee_id, concat(first_name,' ',last_name)full_name, job_id,salary, 
avg(salary) over(partition by department_id) as dept_avg_salary from employees;

-- Show running total of salaries for each department ordered by hire date using SUM()
SELECT *,
       SUM(Salary) OVER (PARTITION BY Department_ID ORDER BY Hire_Date) AS RunningTotal
FROM Employees;

-- Find max salary in each department and compare with employee's salary
SELECT *,
...
insert into employees(employee_id,first_name,last_name,email,phone_number,job_id,commission_pct,manager_id,department_id)
value (11011,'Aditya','Saxena','aditya@cdac.com',213.121,'IT_PROG',0.15,103,60);
SELECT * FROM EMPLOYEES WHERE EMPLOYEE_ID=11011;

delimiter $$
create trigger before_neg_sal_on_emp
before insert on employees
for each row
begin
...
value (11012,'Aditya','Singh','adityaa@cdac.com',213.121,'IT_PROG',-1,0.15,103,60);
...
begin
    ...
    value (concat('A new employee has been added with emp_id:',new.employee_id));
end;;
$$
delimiter ;
show triggers;
...
value (11015,'Addy','Nagarr','addyyyyyy@cdac.com',213.121,curdate(),'IT_PROG',15000,0.15,103,60);
